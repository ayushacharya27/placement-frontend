...
function SetupScreen({ onComplete }) {
  ...
  const [neoId, setNeoId] = useState(''); // 🚀 Added Neo ID state
    ...
    onSuccess: async (codeResponse) => {
      ...
      try {
        const res = await axios.post(`${API_BASE}/auth/google`, { 
          ...
          neo_id: neoId.trim() // 🚀 Pass Neo ID to backend
        });
    ...
    <div className="min-h-screen bg-[#F8F9FA] font-sans flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
        ...
        <div className="mb-3">
          ...
          <input 
            type="text" 
            placeholder="e.g. 23BPS1078" 
            value={regNo}
            onChange={(e) => setRegNo(e.target.value)}
            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-900 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 uppercase placeholder:normal-case placeholder:text-gray-400 bg-gray-50"
          />
        </div>
        ...
        <div className="mb-8">
          ...
          <input 
            type="text" 
            placeholder="e.g. NEO123456" 
            value={neoId}
            onChange={(e) => setNeoId(e.target.value)}
            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-900 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 placeholder:text-gray-400 bg-gray-50"
          />
        </div>

        <button
          onClick={() => {
            if (!regNo.trim() || !neoId.trim()) return alert("Please enter both your Reg Number and Neo ID!");
            login();
          }}
